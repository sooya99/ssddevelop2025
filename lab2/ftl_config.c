...
void InitFTL()
{
	...
	printf("[ ftl configuration complete. ]\r\n");
}
...
void InitChCtlReg()
{
	...
	if(USER_CHANNELS > 1)
		chCtlReg[1] = Addr2Mem(V2FMCRegisters, NSC_1_BASEADDR); /* jy */

	if(USER_CHANNELS > 2)
		chCtlReg[2] = Addr2Mem(V2FMCRegisters, NSC_2_BASEADDR); /* jy */

	if(USER_CHANNELS > 3)
		chCtlReg[3] = Addr2Mem(V2FMCRegisters, NSC_3_BASEADDR); /* jy */

	if(USER_CHANNELS > 4)
		chCtlReg[4] = Addr2Mem(V2FMCRegisters, NSC_4_BASEADDR); /* jy */

	if(USER_CHANNELS > 5)
		chCtlReg[5] = Addr2Mem(V2FMCRegisters, NSC_5_BASEADDR); /* jy */

	if(USER_CHANNELS > 6)
		chCtlReg[6] = Addr2Mem(V2FMCRegisters, NSC_6_BASEADDR); /* jy */
...
void InitNandArray()
{
	...
	printf("[ NAND device reset complete. ]\r\n");
}
